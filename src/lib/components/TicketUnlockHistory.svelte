...
<a href={`/history/${ticket.id_ticket}`} class="block">
<div class="bg-white p-6 mb-2 rounded-xl shadow-md border border-gray-100 transition-all duration-300 hover:shadow-lg"> 
    ...
    <h3 class="text-md font-bold text-gray-900 mb-2">{ticket.ticket_tittle}</h3>
    ...
    <div class="flex items-center justify-between text-sm text-gray-500 pt-4 border-t border-gray-100">
        <div class="flex items-center">
            ...
            src="https://placehold.co/32x32/1e40af/ffffff?text={ticket.cust_name[0]}"
            alt="User avatar"
            />
            <span class="ml-2 font-medium text-gray-700">{ticket.cust_name}</span>
        </div>
    </div>
    <div class="flex items-center justify-between text-sm text-gray-500 pt-4 border-t border-gray-100">
