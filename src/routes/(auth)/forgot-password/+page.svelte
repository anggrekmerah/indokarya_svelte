	...
	const handleEnhance = () => {
		...
		return async ({ result }) => {
			...
			if (result.type === 'failure') {
				errorMessage = result.data?.message || $t('Terjadi kesalahan sistem');
			}
    ...
    <div class="flex items-center justify-center p-4 sm:p-8">
      <div class="w-full max-md py-8 px-4 sm:p-8 md:p-10 bg-white dark:bg-gray-800 rounded-xl shadow-2xl transition duration-500">
        ...
        <div class="space-y-6 text-center">
          ...
          <h2 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">{$t('Forgot Password')}</h2>
          <p class="font-medium text-base text-gray-600 dark:text-gray-400">{$t('Enter your email to receive a reset link')}.</p>
        </div>
